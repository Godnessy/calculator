...
window.addEventListener("keyup", (e) => {
  const key = e.key;
  if (!allowedString.split(" ").includes(key)) {
    return;
  }
...
//working the operator on the numbers set
function operator(sign) {
  operand = sign;
  showUi(operand_ui, sign);
  if (arg2 === "") return;
  const func = operatorMap[sign];
  result = func(Number(arg1), Number(arg2));
  //Checking if the number is a float and if so returning only 3 numbers after the decimal point.
  result = Number(result) % 1 ? result.toFixed(3) : result;
  return result;
}
